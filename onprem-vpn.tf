...
resource "oci_core_ipsec_connection_tunnel_management" "oci-ipsec-connection-tunnel-management-a" {
  ...
  bgp_session_info {
    ...
    customer_interface_ip = "10.10.100.101/30"
    oracle_interface_ip   = "10.10.100.102/30"
  }
...
resource "oci_core_ipsec_connection_tunnel_management" "oci-ipsec-connection-tunnel-management-b" {
  ...
  bgp_session_info {
    customer_bgp_asn      = "1234"
    customer_interface_ip = "10.10.200.201/30"
    oracle_interface_ip   = "10.10.200.202/30"
  }
...
##################################################################################
// OCI VCN tunnel-a
resource "oci_core_drg_attachment_management" "services-hub-vcn-drg-ipsec-attachment-tunnel-a" {
  ...
  drg_id = oci_core_drg.services-hub-vcn-drg.id
  display_name = "OnPrem-DRG-IPSEC-Attachment-Tunnel-A"
  drg_route_table_id = oci_core_drg_route_table.drg-to-fw-on-prem-route-table.id
}

// OCI VCN tunnel-b
resource "oci_core_drg_attachment_management" "services-hub-vcn-drg-ipsec-attachment-tunnel-b" {
  attachment_type = "IPSEC_TUNNEL"
  compartment_id = var.compartment_ocid
  network_id = data.oci_core_ipsec_connection_tunnels.oci-ipsec-connection-tunnels.ip_sec_connection_tunnels.1.id
  drg_id = oci_core_drg.services-hub-vcn-drg.id
  display_name = "OnPrem-DRG-IPSEC-Attachment-Tunnel-B"
  drg_route_table_id = oci_core_drg_route_table.drg-to-fw-on-prem-route-table.id
}
